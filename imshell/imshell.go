...
func NewImageText(replaceText string) *ImageText {
	return &ImageText{text: []rune(replaceText)}
}
...
func (h Hex) ToRGB() (uint8, uint8, uint8) {
	if len(h) > 0 && h[0] == '#' {
		h = h[1:]
	}
	if len(h) == 3 {
...
func RGBToShort(r, g, b uint8) string {
	parts := [3]uint8{r, g, b}
	res := []uint8{}
	dict := createDict()
	incs := [6]uint8{0x00, 0x5f, 0x87, 0xaf, 0xd7, 0xff}
	for _, part := range parts {
		for i := 0; i < len(incs); i++ {
			s := incs[i]
			b := incs[i+1]
			if (s <= part) && (part <= b) {
				sl := math.Abs(float64(s) - float64(part))
				bl := math.Abs(float64(b) - float64(part))
				if sl < bl {
					res = append(res, s)
				} else {
					res = append(res, b)
	...
	hex := string(RGBToHex(res[0], res[1], res[2]))
	return dict[hex]
}
